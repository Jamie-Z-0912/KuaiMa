define("app/article", [ "../mod/base", "../plugs/version", "../plugs/cookieStorage", "../plugs/validread" ], function(require, exports, module) {
    ...
    if (km.gEq("1.1.0") && km.less("1.2.2")) if (Tools.getQueryValue("login") == "1" || Tools.getQueryValue("login") == "0") if (!Storage.get("HASREAD")) {
        Storage.set("HASREAD", 1);
        $("body").css({
            overflow: "hidden",
            height: window.screen.height
        }).append('<div class="km-dialog leader-wrap" id="leaderPage">' + '<div class="con" id="leaderCon">' + '<img id="leaderBg" src="image/leader.png"/>' + '<div class="txt">认真阅读至文章底部<br>才能获得金币奖励哦</div>' + '<div class="btn">知道了</div>' + "</div>" + "</div>");
        $("#leaderCon").css({
            "margin-top": (window.screen.height - $("#leaderCon").height()) / 4,
            "margin-left": (innerWidth - $("#leaderCon").width()) / 2
        });
        $("#leaderPage")[0].addEventListener("touchmove", function(e) {
            e.preventDefault();
            return false;
        }, false);
