define("app/wnl_article", [ "../mod/base", "../plugs/cookieStorage" ], function(require, exports, module) {
    ...
    function doCon() {
        ...
        var mainH = $("#MainCon").height(), per = Tools.getQueryValue("mh") / 100, showH;
        if (!per) {
            if (mainH > innerHeight + 40) {
                showH = innerHeight + 40 + "px";
            } else {
                if (mainH > 250) showH = mainH - 50 + "px"; else showH = mainH + 40 + "px";
            }
        } else {
    ...
    $("#photos li").each(function() {
        var n = Math.ceil(Math.random() * 1e3);
